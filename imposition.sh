...
while [ $page -lt $INTERIORNUMBEROFPAGES ] ; do
	...
	while [ $BOOKLETCURSOR -lt $MIDDLEPAGE ] ; do
		...
		if [[ $BOOKLETCURSOR = 0 ]]; then
			...
			pdftk $firstbookletpagefilename background pointsdagraffe.pdf output $firstbookletpagefilename-marked
...
pdftk /tmp/booklet-{001..$BOOKLETPAGENUMBER}.pdf cat output ${OUTPUTNAME}-prefinal.pdf
pdftk A=${OUTPUTNAME}-prefinal.pdf shuffle AoddWest AevenEast output ${OUTPUTNAME}-CAHIER.pdf
