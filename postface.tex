...
Il n’est pas anodin à cet égard de constater que la musique qui m’accompagna lors de cette rédaction est celle du studio audiomachine, dont le style calibré pour la musique de cinéma est épique. Et plus particulièrement leur album \work{Greatest Hits}\footnote{\textsc{audiomachine} : \work{Greatest Hits}. 2022.} dans lequel je trouvais des accents encore plus violents que les précédents et qui à cet égard me semblèrent propices à être insufflés à la diégèse de cette œuvre.
...
\newcommand\doubledate[2]{\footnotesize#1\\\tiny #2}
...
\begin{tikzpicture}[
    ...
    timeline dashed/.style={thick, dashed},
    event/.style={circle, fill=white, draw=black, inner sep=1.5pt},
    label/.style={font=\small, align=center}
]

% --- Ligne principale ---
% Partie avant l'ellipse
\draw[timeline] (0,0) -- (2.5,0);

% Partie pointillée = ellipse temporelle
\draw[timeline dashed] (3.0,0) -- (5.0,0);

% Partie après l'ellipse
\draw[timeline] (4.5,0) -- (14,0);

% --- Événements ---
\node[event] (ev1) at (0,0) {};
\node[label,above=3pt of ev1] {\doubledate{2025}{-500}};
\node[label,below=3pt of ev1,rotate=-45,anchor=west] {Ère actuelle};

\node[event] (ev2) at (0.8,0) {};
\node[label,above=3pt of ev2] {\doubledate{2525}{0}};
\node[label,below=3pt of ev2,rotate=-45,anchor=west] {Jactum};

% Étiquette pour l'ellipse
\node[label,below=3pt] at (4.0,0) {Histoire tricamillénaire\\de l’Arche};

\node[event] (ev3) at (7.0,0) {};
\node[label,above=3pt of ev3] {\doubledate{31456}{28931}};
\node[label,below=3pt of ev3,rotate=-45,anchor=west] {Moment du récit};

\node[event] (ev4) at (8,0) {};
\node[label,above=3pt of ev4] {\doubledate{31525}{29000}};
\node[label,below=3pt of ev4,rotate=-45,anchor=west] {Arrivée recalculée};

\node[event] (ev5) at (14,0) {};
\node[label,above=3pt of ev5] {\doubledate{32525}{30000}};
\node[label,below=3pt of ev5] {Arrivée initialement prévue\\(au moment du Jactum)};

\end{tikzpicture}
	\end{adjustwidth}
	\caption{Frise chronologique de la civilisation de l’Arche depuis le départ de la Terre jusqu’au événements du récit}
\end{figure}
...
Lors, ce qui advient aux personnages, l’on le devine, est peu enviable. Non contents de se meurtrir les uns les autres de la pire sorte, les humains de l’Arche se nourrissent des chairs d’autres humains \incise{au sens propre}, et de leurs frères de préférence.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.3\textwidth]{structure-de-larche.pdf}
...
Voilà pourquoi il n’est donc pas simplement question de vie, mais de niveau de vie. D’aucun dirait de \xenism{layfe stayle}, sous l’égide du néolibéralisme triomphant avec son cortège de développement personnel et d’idéologie managériale, psalmodiée par ses armées d’influenceurs.
	...
	\resizebox{\textwidth}{!}{%
		...
		\genealogytree[template=signpost,
			...
			child[edges down by = 1 of 6]{% To put edges in the middle: 1/3 + 1/6 = 1/2
              p[tikz={xshift=1.2cm}]{\emph{Dame inconue}}
...
\begin{figure}[!t]
	...
	\marginnote{\null\vspace{0.18cm}\\ \footnotesize\fullcite{FNP-drame}.}
	\centering
